...
class SBSRenderer(
    ...
    // 缓存 Surface，供外部获取（换视频时复用）
    @Volatile var cachedSurface: Surface? = null
        private set
    ...
    private fun recalcVertices() {
        synchronized(this) {
            ...
            val targetAR = if (sbs3dMode) {
                if (rawVideoAR > 2.5f) rawVideoAR / 2f else rawVideoAR
            } else {
                rawVideoAR
            }
            var scaleX = 1f
